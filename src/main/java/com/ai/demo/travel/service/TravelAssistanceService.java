...
public class TravelAssistanceService {
    ...
    private String buildPromptForRecommendations(UserProfileDTO profile) {
        ...
        String rawPrompt = PromptLoader.get(PromptLoader.Prompts.RECOMMENDATIONS);
        return rawPrompt.replace("{{name}}", profile.getName())
                .replace("{{age}}", String.valueOf(age))
                .replace("{{budget_level}}", String.valueOf(profile.getBudgetLevel()))
                .replace("{{preferred_climates}}", String.join(", ", profile.getPreferredClimates()))
                .replace("{{languages_spoken}}", String.join(", ", profile.getLanguagesSpoken()))
                .replace("{{interests}}", String.join(", ", profile.getInterests()))
                .replace("{{travel_style}}", profile.getTravelStyle());
    }
